...
class ScannerController extends Controller
{
    ...
    {   
        ...
        if ($res->count() > 0) {
            ...
            $dtrLogRes2 = DtrLogSubject::create([
                'member_id' => $res->first()->id,
            ...
            if (in_array($res->first()->levels_id, $levels)) {
                $ch = curl_init();
    ...
    {   
        ...
        if ($member->count() > 0) {
            ...
            $url = 'http://'.$_SERVER['SERVER_NAME'].'/samsv4/otp-request?datas='.urlencode(serialize($send_data));
            
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'http://'.$_SERVER['SERVER_NAME'].'/samsv4/otp-request?datas='.urlencode(serialize($send_data)));
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($ch, CURLOPT_HEADER, 0);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    {
        $school_year  = Schoolyear::where('status', 'CURRENT')->first()->id;
        $subjects = Subject::where('schoolyear_id', $school_year)->get();

        $subjectx = array(); 
        foreach ($subjects as $subject) {
            $subjectx[] = array(
                $subject->subjects_id => $subject->subjects_name
            );
        }

        $subjects = array(); $subjectx[] = array('' => 'select a subject');
        foreach($subjectx as $subjectc) {
            foreach($subjectc as $key => $val) {
                $subjects[$key] = $val;
            }
        }
